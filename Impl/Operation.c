...
void SSC_xor16(void* R_ writeto, const void* R_ readfrom)
{
...
void SSC_xor32(void* R_ writeto, const void* R_ readfrom)
{
  uint8_t*       first  = (uint8_t*)      writeto;
  const uint8_t* second = (const uint8_t*)readfrom;
...
void SSC_xor64(void* R_ writeto, const void* R_ readfrom)
{
  uint8_t*       first  = (uint8_t*)      writeto;
  const uint8_t* second = (const uint8_t*)readfrom;
...
void SSC_xor128 (void* R_ writeto, const void* R_ readfrom)
{
  uint8_t*       first  = (uint8_t*)      writeto;
  const uint8_t* second = (const uint8_t*)readfrom;
...
size_t SSC_constTimeMemDiff(const void* R_ v_0, const void* R_ v_1, size_t size)
{
  SSC_ASSERT(v_0 && v_1);
...
bool SSC_isZero(const void* R_ v, size_t n_bytes)
{
  SSC_ASSERT(v);
...
bool SSC_constTimeIsZero(const void* R_ v, size_t n_bytes)
{
  SSC_ASSERT(v);
  const uint8_t* mem = (const uint8_t*)v;
  uint8_t zero_test = 0;
