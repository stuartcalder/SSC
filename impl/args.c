...
static int
short_match_
(const int shortc, R_(const Short_t*) shortv, char ch);

static int
long_match_flag_
(const BAP_Flag_t flag, const int longc, R_(const Long_t*) longv, size_t str_n, R_(char*) str);

BASE_INLINE int
long_match_
(const int longc, R_(const Long_t*) longv, size_t str_n, R_(char*) str)
{ return long_match_flag_(BASE_ARG_PARSER_FLAG_NONE, longc, longv, str_n, str); }

static int
long_match_
(const int, R_(const Long_t*), size_t, R_(char*));

static int
process_shorts_
(const int, R_(char**), const int, R_(const Short_t*), R_(void*));

static int
process_longs_flag_
(const BAP_Flag_t flag, const int argc, R_(char**) argv, const int longc, R_(const Long_t*) longv, R_(void*) state);

BASE_INLINE int
process_longs_
(const int argc, R_(char**) argv, const int longc, R_(const Long_t*) longv, R_(void*) state)
...
int
short_match_(const int shortc, R_(const Short_t*) shortv, char ch)
{
...
int
long_match_flag_
(const BAP_Flag_t  flag , const int    longc,
 R_(const Long_t*) longv, const size_t str_n,
 R_(char*)         str)
{
...
(const int argc,   R_(char**)         argv,
 ...
 R_(void*) state)
{
  if (argc == 0) return 0; /* Are there arguments? */
...
{
  //FIXME
  if (argc == 0) return 0; /* Stop if there are 0 args. */
